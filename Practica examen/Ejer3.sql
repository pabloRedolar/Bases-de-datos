...
SET @pista_mas_reservada = (select id_pista from reservas where year(fecha_reserva) = 2014 group by id_pista having count(*) = (select max(cuenta) from (select COUNT(*) as cuenta from reservas where year(fecha_reserva) = 2014 GROUP BY id_pista)t1) limit 1);
